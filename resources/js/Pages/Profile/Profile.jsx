    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="flex justify-center items-center mt-3 text-atomWhite">
                <a href={user.avatar_url === null ? Placeholder : `../uploads/avatars/${user.avatar_url}`} target="_blank">
                    <img alt="Profile avatar" src={Placeholder} className="w-[200px] rounded-[15px] border-transparent border-2 border-dashed"/>
                </a>
                <div className="ms-2">
                    ...
                    <hr/>
                    <p><span className="font-extrabold">{Cookies.get('language') === 'lt' ? 'Vartotojo kurimo data' : 'User created at'}</span>: {new Date(user.email_verified_at).toLocaleString('lt-LT', {
                        year: 'numeric',
                        month: '2-digit',
                        day: '2-digit',
                    })}</p>
            ...
            {threads.length > 0 ?        
                <div className="flex justify-center mt-4 text-atomWhite row font-extrabold">
                    {Cookies.get('language') === 'lt' ? 'Vartotojo įrašai' : "User's threads"}
                        {threads.map(thread => (
